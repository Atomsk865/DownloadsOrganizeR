...
try {
    ...
    if (Test-Path $sourceFile) {
        ...
        if (Test-Path $configSource) {
            ...
            # ENTERPRISE SAFETY: Set organizer_enabled to false by default
            # Admin must explicitly enable via Dashboard after completing setup
            try {
...
"@
    ...
    Write-Host "    INSTALLATION COMPLETE - NEXT STEPS REQUIRED" -ForegroundColor Green
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "⚠️  IMPORTANT: The organizer service is currently DISABLED" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "For enterprise safety, the service will NOT organize files until you:" -ForegroundColor White
    Write-Host "  1. Open the Dashboard at http://localhost:5000" -ForegroundColor White
    Write-Host "  2. Complete the setup wizard (create admin account)" -ForegroundColor White
    Write-Host "  3. Configure watch folders and destination settings" -ForegroundColor White
    Write-Host "  4. Review your configuration carefully" -ForegroundColor White
    Write-Host "  5. Enable the organizer service from Settings" -ForegroundColor White
    Write-Host "  6. Restart the service to begin monitoring" -ForegroundColor White
    Write-Host ""
    Write-Host "This prevents accidental file organization before proper setup." -ForegroundColor Gray
    Write-Host ""
    Write-Host "Service Name: $ServiceName" -ForegroundColor Cyan
    Write-Host "Service Status: Running (but organizer is disabled)" -ForegroundColor Cyan
    Write-Host "Configuration: $configDest" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host ""
}
