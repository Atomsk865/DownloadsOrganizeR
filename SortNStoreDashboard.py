...
def get_installation_paths():
    """Detect installation and data directories based on script location or config.
    
    Returns tuple: (install_dir, data_dir, config_dir, log_dir)
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    install_marker = os.path.join(script_dir, ".install_path")
    
    # Try to read from marker file (created during installation)
        ...
        try:
            with open(install_marker, 'r') as f:
                ...
                try:
                    ...
                    config_dir = paths.get('config_dir', os.path.join(data_dir, 'config'))
                    log_dir = paths.get('log_dir', os.path.join(data_dir, 'logs'))
                    
                    if os.path.isdir(install_dir):
                        return install_dir, data_dir, config_dir, log_dir
                except (json.JSONDecodeError, ValueError):
                    # Old format: just the path as plain text
                    if os.path.isdir(content):
                        # Use old behavior: everything in install dir
                        return content, content, content, os.path.join(content, "service-logs")
        except Exception:
            pass
    
    # Fallback: determine based on installation location
    # If in Program Files, use ProgramData for data/config/logs
    if "Program Files" in script_dir:
        data_dir = r"C:\ProgramData\DownloadsOrganizeR"
        return script_dir, data_dir, os.path.join(data_dir, "config"), os.path.join(data_dir, "logs")
    
    # Otherwise use script directory for everything (simple/legacy mode)
    return script_dir, script_dir, script_dir, os.path.join(script_dir, "service-logs")

INSTALL_DIR, DATA_DIR, CONFIG_DIR, LOG_DIR = get_installation_paths()
DEFAULT_LOGS_DIR = LOG_DIR

# --- Service and Config ---
SERVICE_NAME = "SortNStore"

# Config files location: use config directory if separate from install, otherwise install dir
_config_base_dir = CONFIG_DIR if CONFIG_DIR != INSTALL_DIR else INSTALL_DIR
CONFIG_FILE = os.path.join(_config_base_dir, "sortnstore_config.json")
DASHBOARD_CONFIG_FILE = os.path.join(_config_base_dir, "dashboard_config.json")
...
DEFAULT_CONFIG = {
    ...
    "logs_dir": DEFAULT_LOGS_DIR,  # Dynamically set based on installation location
