...
<#
Install-And-Monitor-SortNStoreService.ps1
- Purpose: Installs NSSM, configures the SortNStore service, and sets up health monitoring.
...
try {
    ...
    # Copy SortNStoreService.py (new name) with legacy Organizer.py shim alongside
    $serviceSource = Join-Path $PSScriptRoot "SortNStoreService.py"
    $serviceDest   = Join-Path $ScriptsRoot "SortNStoreService.py"
    $legacyShim    = Join-Path $PSScriptRoot "Organizer.py"
    if (Test-Path $serviceSource) {
        Copy-Item -Path $serviceSource -Destination $serviceDest -Force
        Write-Host "Copied SortNStoreService.py to $ScriptsRoot" -ForegroundColor Green
        if (Test-Path $legacyShim) {
            Copy-Item -Path $legacyShim -Destination (Join-Path $ScriptsRoot "Organizer.py") -Force
            Write-Host "Copied Organizer.py shim for backward compatibility" -ForegroundColor Green
        }
    } else {
        throw "SortNStoreService.py not found in $PSScriptRoot. Please place it in the same folder as this script."
    }
        # Copy configuration (prefer sortnstore_config.json, fall back to organizer_config.json)
        $configSource = $null
        if (Test-Path (Join-Path $PSScriptRoot "sortnstore_config.json")) {
            $configSource = Join-Path $PSScriptRoot "sortnstore_config.json"
        } elseif (Test-Path (Join-Path $PSScriptRoot "organizer_config.json")) {
            $configSource = Join-Path $PSScriptRoot "organizer_config.json"
        }
        if ($configSource) {
            $configDest = Join-Path $ScriptsRoot (Split-Path -Leaf $configSource)
            Copy-Item -Path $configSource -Destination $configDest -Force
            Write-Host "Copied $(Split-Path -Leaf $configSource) to $ScriptsRoot" -ForegroundColor Green
        } else {
            Write-Host "No config file found; continuing with defaults." -ForegroundColor Yellow
        }
    ...
    $ConfigPath = $null
    if (Test-Path (Join-Path $ScriptsRoot "sortnstore_config.json")) {
        $ConfigPath = Join-Path $ScriptsRoot "sortnstore_config.json"
    } elseif (Test-Path (Join-Path $ScriptsRoot "organizer_config.json")) {
        $ConfigPath = Join-Path $ScriptsRoot "organizer_config.json"
    }
    ...
    if ($ConfigPath) {
        try {
            ...
            Write-Host "Warning: failed to parse $ConfigPath: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }

    if (-not $PSBoundParameters.ContainsKey('ServiceName') -and $ConfiguredServiceName) {
        $DesiredServiceName = $ConfiguredServiceName
        Write-Host "Using service name from config: $DesiredServiceName" -ForegroundColor Cyan
    } else {
    ...
    $StdOutLog = Join-Path $LogsRoot "sortnstore_stdout.log"
    $StdErrLog = Join-Path $LogsRoot "sortnstore_stderr.log"
    ...
    & $NssmExe set $ServiceName Description "Organizes Downloads folder in real-time using SortNStoreService"
    ...
    # -------------------------
    $MonitorScriptPath = Join-Path $ScriptsRoot "Monitor-SortNStoreService.ps1"
    $MonitorScript = @"
param(
